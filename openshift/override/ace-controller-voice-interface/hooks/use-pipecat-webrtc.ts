...
//
// Forked from ace-controller examples/webrtc_ui: first WebSocket message includes iceServers so the
// pipeline Python peer uses the same Metered credentials as the browser (avoids aioice STUN 401 on
// CHANNEL_BIND when the server would otherwise fetch a second, different REST response).
  ...
  const connect = useCallback(async () => {
    try {
      ...
      const answer = await new Promise<RTCSessionDescriptionInit>((resolve, reject) => {
          ...
          if (offer) {
            ws.send(
              JSON.stringify({
                sdp: offer.sdp,
                type: offer.type,
                iceServers: params.rtcConfig?.iceServers ?? [],
              }),
            );
          }
        };
        ...
        ws.onmessage = (event) => {
          resolve(JSON.parse(event.data) as RTCSessionDescriptionInit);
        };
